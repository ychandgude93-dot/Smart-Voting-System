...
1.	python-3.8+
2.	pycharm-community-2019.3.1 (optional)
3.	MySQL server or compatible (local or remote)
...
4. (Optional) Generate an admin password hash and set `ADMIN_PASSWORD_HASH` in `.env`:
   - python code/generate_admin_hash.py yourpassword
   - copy the printed hash into `ADMIN_PASSWORD_HASH`
5. Run the app:
   - set FLASK_DEBUG=1 (only for development) and `python code/main.py`

**Security notes:**
- Do not commit `.env` to version control. Use a secrets manager in production.
- Ensure `SESSION_COOKIE_SECURE=1` and run behind HTTPS in production.
...
### NOTE: All special instructions to be followed to run the project are mentioned in the project documentation.

**Security note:** Credentials and passwords should NOT be stored in plaintext or checked into version control. Use environment variables or a secrets manager to store sensitive values (see `.env.example`).
